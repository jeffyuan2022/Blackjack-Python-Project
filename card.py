class Card:
    """
    Card class.

    # Doctests for str and repr
    >>> card_1 = Card("A", "spades")
    >>> print(card_1)
    ____
    |A  |
    | ♠ |
    |__A|
    >>> card_1
    (A, spades)
    >>> card_2 = Card("K", "spades")
    >>> print(card_2)
    ____
    |K  |
    | ♠ |
    |__K|
    >>> card_2
    (K, spades)
    >>> card_3 = Card("A", "diamonds")
    >>> print(card_3)
    ____
    |A  |
    | ♦ |
    |__A|
    >>> card_3
    (A, diamonds)

    # Doctests for comparisons
    >>> card_1 < card_2
    False
    >>> card_1 > card_2
    True
    >>> card_3 > card_1
    False

    # Doctests for set_visible()
    >>> card_3.set_visible(False)
    >>> print(card_3)
    ____
    |?  |
    | ? |
    |__?|
    >>> card_3
    (?, ?)
    >>> card_3.set_visible(True)
    >>> print(card_3)
    ____
    |A  |
    | ♦ |
    |__A|
    >>> card_3
    (A, diamonds)

    >>> Card4 = Card(2, "diamonds") 
    >>> Card5 = Card("A", "spades")
    >>> Card5 < Card4
    False
    """

    # Class Attribute(s)
    suit_sorted = ["clubs", "diamonds", "hearts", "spades"]
    rank_sorted = list(range(2,11)) + ["J", "Q", "K", "A"]

    def __init__(self, rank, suit, visible=True):
        """
        Creates a card instance and asserts that the rank and suit are valid.
        """
        self.rank = rank
        assert rank in range(2, 11) or (rank in ["K", "Q", "A", "J"])
        self.suit = suit
        assert suit in ["hearts", "spades", "clubs", "diamonds"]
        self.visible = visible
        assert type(visible) == bool
        

    def __lt__(self, other_card):
        if self.rank_sorted.index(self.rank) == self.rank_sorted.index(other_card.rank):
            if self.suit_sorted.index(self.suit) < self.suit_sorted.index(other_card.suit):
                return True
            else:
                return False
        elif self.rank_sorted.index(self.rank) < self.rank_sorted.index(other_card.rank):
            return True
        else:
            return False

    def __str__(self):
        """
        Returns ASCII art of a card with the rank and suit. If the card is
        hidden, question marks are put in place of the actual rank and suit.

        Examples:
        ____
        |A  |
        | ♠ |
        |__A|
        ____
        |?  |
        | ? |
        |__?|             
        """
        if self.suit == "hearts":
            symbol = "♥"
        elif self.suit == "spades":
            symbol = "♠"
        elif self.suit == "clubs":
            symbol = "♣"
        elif self.suit == "diamonds":
            symbol = "♦"
        if self.visible == True:                                        
            return f"____\n|{self.rank}  |\n| {symbol} |\n|__{self.rank}|"
        else:
            return f"____\n|?  |\n| ? |\n|__?|"

    def __repr__(self):
        """
        Returns (<rank>, <suit>). If the card is hidden, question marks are
        put in place of the actual rank and suit.           
        """
        if self.visible == True:
            return f"({self.rank}, {self.suit})"
        else:
            return f"(?, ?)"

    def get_rank(self):
        return self.rank
    
    def get_suit(self):
        return self.suit

    def set_visible(self, visible):
        assert type(visible) == bool
        self.visible = visible